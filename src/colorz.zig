        ...
        while (i < fmt.len) {
            ...
            switch (fmt[i]) {
                '@' => {
                    ...
                    const ansi_seq = CommandCodes.get(code) orelse @compileError("|@" ++ code ++ "| is not a valid color code");
                    literal = literal ++ ansi_seq;
                    i += code.len + 1;
                },
                '!' => {
                    const code = until('}', fmt[i..]);
                    switch (code[1]) {
                        'G' => {
                            ...
                            const rowt = code[2..sep];
                            const colt = code[sep + 1 ..];
                            const row = std.fmt.parseInt(usize, rowt, 10) catch @compileError("Invalid row: " ++ code[2..] ++ " is not parseable");
                            const col = std.fmt.parseInt(usize, colt, 10) catch @compileError("Invalid col: " ++ code[2..] ++ " is not parseable");
                            literal = literal ++ comptimePrint(esc.E.GOTO, .{ row, col });
                        },
                        else => {
                            const ansi_seq = BangCodes.get(code) orelse @compileError("|!" ++ code ++ "| is not a valid bang code");
                            literal = literal ++ ansi_seq;
                        },
                    }
                    i += code.len + 1;
...
test {
    const test_cases: []const [2][]const u8 = &.{
        ...
        // .{ "Hello, {@green}{}{!", "Hello, \x1b[32m{}\x1b[H" }, // expected to fail
    };
